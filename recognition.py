...
# DBENGINE = create_engine("mysql+pymysql://root:raspberry@localhost/FR_API")
DBENGINE = create_engine("mysql+pymysql://root@localhost/FR_API")
...
def findmysql(
    ...
    db_representations = DBSESSION.execute(text("""SELECT `image_location`, `pickle_data`, TRIM(LEADING '[' FROM TRIM(TRAILING ']' FROM `pickle_data`)) as pickle FROM representations"""))
    for rep in db_representations :
        pickle_data = json.loads(rep[1])
        print(pickle_data)
        print('')
        for image in pickle_data:
...
def __find_bulk_embeddings(
    ...
    for employee in tqdm(
        ...
        employee = windows_to_linux_path(employee)
        file_hash = image_utils.find_image_hash(employee)
        file_location = str(employee)
    ...
    if to_mysql: 
        with DBSESSION as connection :
            connection.execute(
                text(
                    """
                        INSERT INTO representations (`image_location`, `pickle_data`) 
                        VALUES ('"""+str(file_location)+"""', '"""+json.dumps(representations)+"""')
                    """
                )
            )
            connection.commit()
...
def to_pickle(
    ...
    db_representations = DBSESSION.execute(text("""SELECT `image_location`, `pickle_data` FROM representations"""))
    for rep in db_representations :
        pickle_data = json.loads(rep[1])
        if not silent:
            print(pickle_data)
    ...
    if not silent:
        toc = time.time()
        logger.info(f"to pickle function duration {toc - tic} seconds")
